...
function renderChatContent(){
  ...
  content.innerHTML=`<header class="content-header"><button class="icon-btn mobile-back" data-action="mobile-back" aria-label="Back">${icon('back')}</button>${headerAvatar(chat)}<div class="content-heading"><h2>${esc(chat.name)} ${chat.pinned?`<span class="header-pin" title="Pinned">${icon('pinFilled')}</span>`:''}</h2><p>${chat.archived?'Archived conversation':'Personal conversation'}${messageSearch?` · Searching “${esc(messageSearch)}”`:''}</p></div><div class="content-actions"><button class="icon-btn optional-mobile" data-action="search-messages" title="Search conversation" aria-label="Search conversation">${icon('search')}</button><button class="icon-btn hide-narrow ${chat.pinned?'active-pin':''}" data-action="pin-chat" data-id="${chat.id}" title="${chat.pinned?'Unpin':'Pin'}" aria-label="${chat.pinned?'Unpin':'Pin'} conversation">${icon(chat.pinned?'pinFilled':'pin')}</button><button class="icon-btn" data-action="refresh-cloud" title="Refresh latest GitHub data" aria-label="Refresh latest GitHub data">${icon('refresh')}</button><button class="icon-btn" data-action="chat-menu" data-id="${chat.id}" title="Conversation options" aria-label="Conversation options">${icon('more')}</button></div></header><div class="content-scroll" id="messageScroll"><div class="message-list">${html}</div></div>${recording?recordingHTML():composerHTML()}`;
  bindComposer();
