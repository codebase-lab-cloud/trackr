...
function openModal({title,subtitle='',body='',footer='',wide=false,locked=false,onOpen}={}){
  const root=$('#modalRoot');root.innerHTML=`<div class="modal-overlay" data-modal-overlay><section class="modal-card ${wide?'wide':''}" role="dialog" aria-modal="true" aria-label="${esc(title)}"><header class="modal-head"><div class="modal-head-copy"><h2>${esc(title)}</h2>${subtitle?`<p>${esc(subtitle)}</p>`:''}</div>${locked?'':`<button class="modal-close" data-close-modal aria-label="Close">×</button>`}</header><div class="modal-body">${body}</div>${footer?`<footer class="modal-footer">${footer}</footer>`:''}</section></div>`;
...
function actionMenu(title,items){menuActions=items;openModal({title,body:`<div class="menu-list">${items.map((it,i)=>`<button class="menu-item ${it.danger?'danger':''}" data-menu-action="${i}"><span class="menu-icon">${it.icon||'•'}</span><span>${esc(it.label)}</span></button>`).join('')}</div>`});}
...
function avatarStyle(item){ return `background:${esc(item.avatar?'#fff':(item.color||COLORS[0]))};color:#fff`; }
...
function sidebarHeader(title,subtitle,action,label,placeholder){
  return `<div class="sidebar-head"><div class="sidebar-title-line"><div><h1>${esc(title)}</h1><div class="sidebar-sub">${esc(subtitle)}</div></div>${action?`<button class="sidebar-create" data-action="${action}" aria-label="${esc(label)}">+</button>`:''}</div>${placeholder?`<div class="side-search"><span>⌕</span><input id="sideSearch" value="${esc(searchText)}" placeholder="${esc(placeholder)}"></div>`:''}</div>`;
}
function renderChatSidebar(){
  ...
  const row=c=>{const last=lastMessage(c.id);const pv=last?(last.kind==='voice'?'Voice note':preview(last.text)):'No entries yet';return `<button class="side-row ${c.id===state.activeChatId?'active':''}" data-action="select-chat" data-id="${c.id}"><span class="side-avatar" style="${avatarStyle(c)}">${avatarInner(c)}</span><span class="side-copy"><span class="side-name">${esc(c.name)} ${c.pinned?'<span class="side-pin">◆</span>':''}</span><span class="side-preview">${esc(pv)}</span></span><span class="side-time">${last?esc(lastUpdatedLabel(last.createdAt)):''}</span></button>`};
  ...
  $('#sidebarFooter').innerHTML=`<div class="sidebar-footer"><button class="sidebar-footer-btn" data-action="toggle-archived">${showArchived?'← Back to conversations':`▱ Archived${archivedCount?` · ${archivedCount}`:''}`}</button></div>`;
  bindSideSearch();
}
function renderTrackerSidebar(){
  ...
  $('#sidebarFooter').innerHTML='<div class="sidebar-footer"><button class="sidebar-footer-btn" data-action="new-tracker">＋ Create another tracker</button></div>';
  bindSideSearch();
}
function renderSettingsSidebar(){
  $('#sidebarHeader').innerHTML=sidebarHeader('Settings','GitHub connection and shared JSON',null,null,null);
  $('#sidebarContent').innerHTML=`<div class="side-section-title">Workspace</div><button class="side-row active"><span class="side-avatar" style="background:#fff;color:#3f0e40">⚙</span><span class="side-copy"><span class="side-name">Settings</span><span class="side-preview">Backup, security and install</span></span></button>`;
  $('#sidebarFooter').innerHTML='<div class="sidebar-footer"><button class="sidebar-footer-btn" data-action="lock">⌁ Lock workspace now</button></div>';
}
...
function renderChatContent(){
  ...
  if(!chat){ content.innerHTML=`<div class="empty-main"><div class="empty-card"><div class="empty-illustration">☵</div><h2>No conversation selected</h2><p>Create a conversation for groceries, daily tasks, work, general notes, or anything else.</p><button class="btn primary" data-action="new-chat">Create conversation</button></div></div>`;return; }
  ...
  if(!messages.length) html=`<div class="empty-main"><div class="empty-card"><div class="empty-illustration">${messageSearch?'⌕':'✎'}</div><h2>${messageSearch?'No matching entries':'Start writing'}</h2><p>${messageSearch?'Try a different search.':'This conversation is yours. Add a thought, list, task, or voice note.'}</p></div></div>`;
  content.innerHTML=`<header class="content-header"><button class="icon-btn mobile-back" data-action="mobile-back" aria-label="Back">←</button>${headerAvatar(chat)}<div class="content-heading"><h2>${esc(chat.name)} ${chat.pinned?'<span title="Pinned">◆</span>':''}</h2><p>${chat.archived?'Archived conversation':'Personal conversation'}${messageSearch?` · Searching “${esc(messageSearch)}”`:''}</p></div><div class="content-actions"><button class="icon-btn optional-mobile" data-action="search-messages" title="Search conversation">⌕</button><button class="icon-btn hide-narrow" data-action="pin-chat" data-id="${chat.id}" title="${chat.pinned?'Unpin':'Pin'}">◆</button><button class="icon-btn" data-action="chat-menu" data-id="${chat.id}" title="Conversation options">•••</button></div></header><div class="content-scroll" id="messageScroll"><div class="message-list">${html}</div></div>${recording?recordingHTML():composerHTML()}`;
  bindComposer();
  if(!messageSearch) requestAnimationFrame(()=>{const s=$('#messageScroll');if(s)s.scrollTop=s.scrollHeight;});
}
function messageHTML(m){
  const body=m.kind==='voice'?`<div class="voice-note"><span class="voice-note-icon">●</span><div class="voice-note-copy"><div class="voice-note-title">Voice note</div><audio controls playsinline preload="metadata" src="${esc(m.audio)}"></audio></div><span class="voice-duration">${formatDuration(m.duration||0)}</span></div>`:`<div class="message-text">${nl(m.text)}</div>`;
  return `<article class="message-row"><span class="message-user">YOU</span><div class="message-body"><div class="message-meta"><span class="message-author">You</span><span class="message-time">${esc(localeTime(m.createdAt))}</span>${m.editedAt?'<span class="edited">edited</span>':''}${m.starred?'<span class="star-mark" title="Starred">★</span>':''}</div>${body}</div><button class="message-menu" data-action="message-menu" data-id="${m.id}" aria-label="Entry options">•••</button></article>`;
}
function composerHTML(){ return `<footer class="message-composer"><div class="composer-box"><textarea id="composerInput" class="composer-input" rows="1" placeholder="Write in ${esc(activeChat()?.name||'conversation')}…"></textarea><div class="composer-tools"><button class="composer-tool" data-action="voice-record" title="Record voice note" aria-label="Record voice note">🎙</button><button class="composer-tool" data-action="composer-tip" title="Writing tips">Aa</button><span class="composer-spacer"></span><button id="sendText" class="composer-send" title="Send" aria-label="Send" disabled>➤</button></div></div></footer>`; }
...
function renderTrackerContent(){
  ...
  if(!tracker){content.innerHTML=`<div class="empty-main"><div class="empty-card"><div class="empty-illustration">▤</div><h2>Create your first tracker</h2><p>Add categories and topics for learning, health, projects, or any system you want to build.</p><button class="btn primary" data-action="new-tracker">Create tracker</button></div></div>`;return;}
...
function chatMenu(chat){
  actionMenu(chat.name,[
    {icon:'✎',label:'Rename or change picture',fn:()=>chatDialog(chat)},
    {icon:'◆',label:chat.pinned?'Unpin conversation':'Pin conversation',fn:async()=>{chat.pinned=!chat.pinned;chat.updatedAt=now();await persistState();render();toast(chat.pinned?'Conversation pinned':'Conversation unpinned')}},
    {icon:'▱',label:chat.archived?'Unarchive conversation':'Archive conversation',fn:async()=>{chat.archived=!chat.archived;chat.updatedAt=now();if(chat.archived&&!showArchived)state.activeChatId=state.chats.find(c=>!c.archived)?.id||null;await persistState();render();toast(chat.archived?'Conversation archived':'Conversation restored')}},
    {icon:'⌕',label:'Search this conversation',fn:()=>searchMessagesDialog()},
    {icon:'×',label:'Delete conversation',danger:true,fn:async()=>{if(await confirmModal('Delete conversation?',`“${chat.name}” and all of its entries and voice notes will be permanently deleted.`)){state.chats=state.chats.filter(c=>c.id!==chat.id);state.messages=state.messages.filter(m=>m.chatId!==chat.id);state.activeChatId=state.chats.find(c=>!c.archived)?.id||state.chats[0]?.id||null;await persistState();render();toast('Conversation deleted')}}}
  ]);
}
...
function messageMenu(message){
  actionMenu(message.kind==='voice'?'Voice note':'Entry',[
    ...(message.kind==='text'?[{icon:'✎',label:'Edit entry',fn:()=>editMessage(message)},{icon:'⧉',label:'Copy text',fn:async()=>{try{await navigator.clipboard.writeText(message.text);toast('Copied')}catch{toast('Copy is not available in this browser')}}}]:[]),
    {icon:'★',label:message.starred?'Remove star':'Star entry',fn:async()=>{message.starred=!message.starred;await persistState();renderContent();toast(message.starred?'Entry starred':'Star removed')}},
    {icon:'×',label:'Delete entry',danger:true,fn:async()=>{if(await confirmModal('Delete entry?','This entry will be permanently removed.')){state.messages=state.messages.filter(m=>m.id!==message.id);const c=activeChat();if(c)c.updatedAt=now();await persistState();render();toast('Entry deleted')}}}
  ]);
}
...
function trackerMenu(tracker){
  actionMenu(tracker.name,[
    {icon:'✎',label:'Edit tracker',fn:()=>trackerDialog(tracker)},
...
function categoryMenu(tracker,category){
  ...
  actionMenu(category.name,[
    {icon:'✎',label:'Edit category and notes',fn:()=>categoryDialog(category)},
    ...(i>0?[{icon:'←',label:'Move category left',fn:async()=>{[tracker.categories[i-1],tracker.categories[i]]=[tracker.categories[i],tracker.categories[i-1]];await persistState();renderContent()}}]:[]),
    ...(i<tracker.categories.length-1?[{icon:'→',label:'Move category right',fn:async()=>{[tracker.categories[i+1],tracker.categories[i]]=[tracker.categories[i],tracker.categories[i+1]];await persistState();renderContent()}}]:[]),
    {icon:'×',label:'Delete category',danger:true,fn:async()=>{const detail=category.topics.length?` It contains ${category.topics.length} topic${category.topics.length===1?'':'s'}.`:'';if(await confirmModal('Delete category?',`“${category.name}” will be permanently deleted.${detail}`)){tracker.categories=tracker.categories.filter(c=>c.id!==category.id);tracker.updatedAt=now();await persistState();render();toast('Category deleted')}}}
  ]);
}
...
function topicMenu(tracker,category,topic){
  ...
  actionMenu(topic.name,[
    {icon:'✎',label:'Edit topic and notes',fn:()=>topicDialog(category,topic)},
    ...(tracker.categories.length>1?[{icon:'↗',label:'Move to another category',fn:()=>moveTopicDialog(tracker,category,topic)}]:[]),
    ...(i>0?[{icon:'↑',label:'Move topic up',fn:async()=>{[category.topics[i-1],category.topics[i]]=[category.topics[i],category.topics[i-1]];await persistState();renderContent()}}]:[]),
    ...(i<category.topics.length-1?[{icon:'↓',label:'Move topic down',fn:async()=>{[category.topics[i+1],category.topics[i]]=[category.topics[i],category.topics[i+1]];await persistState();renderContent()}}]:[]),
    {icon:'×',label:'Delete topic',danger:true,fn:async()=>{if(await confirmModal('Delete topic?',`“${topic.name}” and its notes will be permanently deleted.`)){category.topics=category.topics.filter(t=>t.id!==topic.id);tracker.updatedAt=now();await persistState();render();toast('Topic deleted')}}}
  ]);
}
...
$('#lockButton').onclick=lockApp;$('#openSettings').onclick=()=>setMode('settings',true);$('#installHelp').onclick=installHelpDialog;$('#globalSearchButton').onclick=globalSearchDialog;
